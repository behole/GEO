#!/usr/bin/env python3
"""
GEO Optimization System - Umbrella Command (Fixed)
Main entry point for running the complete GEO optimization system

Usage:
    python run_geo_system_fixed.py --mode [full|discovery|content|competitive|monitoring]
...
async def run_agent_1():
    ...
    try:
        ...
        sys.path.insert(0, os.path.join(os.getcwd(), 'discovery_baseline_agent'))
        ...
        # Clean up path
        if os.path.join(os.getcwd(), 'discovery_baseline_agent') in sys.path:
            sys.path.remove(os.path.join(os.getcwd(), 'discovery_baseline_agent'))

async def run_agent_2():
    """Run Agent 2 - Content Analysis Agent"""
    try:
        # Add the agent directory to Python path
        sys.path.insert(0, os.path.join(os.getcwd(), 'content_analysis_agent'))
        ...
        # Clean up path
        if os.path.join(os.getcwd(), 'content_analysis_agent') in sys.path:
            sys.path.remove(os.path.join(os.getcwd(), 'content_analysis_agent'))
...
async def run_full_system():
    ...
    # For Agent 4, check if we have results from previous agents
    has_prior_results = any(result is not None for result in results.values())
    if has_prior_results:
        results['agent_4'] = await run_agent_4()
    else:
        logger.info("🔄 Running Agent 4 in test mode (no successful prior agents)")
        results['agent_4'] = await run_agent_4(test_mode=True)
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="GEO Optimization System - Complete Analysis Suite (Fixed)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python run_geo_system_fixed.py --mode full              # Run all agents sequentially
  python run_geo_system_fixed.py --mode discovery         # Run only Agent 1
  python run_geo_system_fixed.py --mode content           # Run only Agent 2
  python run_geo_system_fixed.py --mode competitive       # Run only Agent 3
  python run_geo_system_fixed.py --mode monitoring        # Run only Agent 4
  python run_geo_system_fixed.py --continuous --interval 6 # Start continuous monitoring
  python run_geo_system_fixed.py --status                 # Check system status
  python run_geo_system_fixed.py --stop                   # Stop continuous monitoring
        """
    )
    ...
    if args.mode == 'discovery':
        ...
        await run_agent_4(test_mode=True)  # Use test mode for standalone runs
    elif args.mode == 'full':
